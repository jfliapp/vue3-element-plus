<template>
    <div class="container"> 
        ...
        <div class="content">
            <div class="form_data">
                <el-form ref='formRef' :disabled="$route.query.type == 'see'?true:false" :model="partnerInfo" :rules="partnerRules" class="" label-width="180px">
                <!-- <el-form ref='formRef' :model="partnerInfo" :rules="partnerRules" class="" label-width="180px"> -->
                    <el-row >
                        <el-col :span="12">
                    ...
                    <el-row >
                        <el-col :span="12">
                            <el-form-item label="TP积分奖励总金额">
                                <el-input v-model="partnerInfo.pointsTpAmount.decimalFourString" placeholder="请输入TP积分奖励总金额" class="headInput" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="[合作商]积分奖励总金额">
                                <el-input v-model="partnerInfo.pointsPartnerAmount.decimalFourString" placeholder="请输入[合作商]积分奖励总金额" class="headInput" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row >
                        <el-col :span="12">
                            <el-form-item label="PRO积分奖励总金额">
                                <el-input v-model="partnerInfo.pointsProAmount.decimalFourString" placeholder="请输入PRO积分奖励总金额" class="headInput" />
                            </el-form-item>
                        </el-col>
                    </el-row>
                </el-form>
            ...
            <div class="table_box">
                ...
                <el-table v-if="$route.query.type == 'see'?true:false" :data="tableData" border style="width: 100%">
                    ...
                    <el-table-column prop="tpAmount.decimalFourString" label="TP积分" align="center" />
                    <el-table-column prop="partnerAmount.decimalFourString" label="合作商积分" align="center" />
                    <el-table-column prop="proAmount.decimalFourString" label="PRO积分" align="center" />
...
import { searchPointsDetail } from '@/api/autarkyManage/integralManage'
...
export default {
    ...
    methods: {
        ...
        backShop() {
            this.$router.push({
                path: '/self-support/integralManage',
            })
        },
        // 查询详情
        getPartnerDetail() {
            searchPointsDetail({ orderNo: this.detailList.orderNo }).then(res => {
                console.log(res)
                if (res.success) {
                    this.partnerInfo = res.data
                    this.tableData = res.data.pointsItemList
                    // this.partnerInfo.name = res.data.name
                    // this.partnerInfo.shortName = res.data.shortName
                    // this.partnerInfo.province = res.data.province
                    // this.getCity(this.partnerInfo.province)
                    // this.firstCity = res.data.city
                    // this.partnerInfo.contactName = res.data.contactName
                    // this.partnerInfo.contactPhone = res.data.contactPhone
                    // this.partnerInfo.settlementCycle = res.data.settlementCycle
                    // this.partnerInfo.integralName = res.data.integralName
                    // this.partnerInfo.accountNo = res.data.accountNo
                    // this.partnerInfo.assetsType = res.data.assetsType
                    // this.imgList = [
                    //     {
                    //         url: res.data.licensePhoto
                    //     }]
                    // this.imageUrl = res.data.licensePhoto
                }
            })
        },
        // 图片上传
